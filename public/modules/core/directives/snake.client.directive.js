...
angular.module('core').directive("welloSnake", ['$timeout', function($timeout){
  return {
    restrict: "E",
    replace: true,
    scope: {
    ...
    link: function(scope, element, attrs){
      ...
      scope.goFullscreen = function () {
          ...
          } else if (elem.msRequestFullscreen) { // IE current API
      ...
      var AI_SNAKE_GATHERER_TYPE = 1; // (Target Food)
      var AI_SNAKE_ASSASSIN_TYPE = 2; // (Target Player Head)
      var AI_SNAKE_CLOSEST_TYPE = 3; // (Target Food or Player Head -- Closest)
      var AI_SNAKE_FARTHEST_TYPE = 4; // (Target Food or Player Head -- Farthest)
      // Background color
      var CLEAR_COLOR = '#00F';
      ...
      var DEFAULT_SPEED = 0; // slow
      var DEFAULT_DIMENSION = 1; // medium
      var DEFAULT_AI_SNAKES = 2; // any more is trouble for a beginner
      var DEFAULT_START_LENGTH = 2; // 20
      var DEFAULT_EXPAND_LENGTH = 2; // 10
      var DEFAULT_CANDY_BUFFER = 2; // 2, decently away from walls

      /* VARIABLES */

      /* Main data structures */ 
      var playing_field; // hash table
      var player_snake; // circularQueue
      var ai_snake; // array of circularQueues
      var candy; // special Collidable
      ...
      var speeds = [80.0,60.0,40.0,20.0];
      var dimensions = [[40,30],[60,45],[80,60]]
      var starting_lengths = [5,10,20,50];
      var expand_lengths = [1,5,10,50];
      var candy_buffers = [0,1,2,3];
      scope.ai_snake_options = [{value:0,name:"0 (Trivial)"},{value:1,name:"1 (Simple)"},{value:2,name:"2 (Beginner)"},{value:3,name:"3 (Recommended)"},{value:4,name:"4 (Hard)"},{value:5,name:"5 (Very Hard)"},{value:6,name:"6 (Yikes!)"},{value:7,name:"7 (Frustrating)"}]
      scope.speed_options = [{value:0,name:"Slow (Beginner)"},{value:1,name:"Medium (Recommended)"},{value:2,name:"Fast (Hard)"},{value:3,name:"Twiddle Fingers (Impossible)"}]
      scope.dimension_options = [{value:0,name:"Tiny (Hard)"},{value:1,name:"Regular (Recommended)"},{value:2,name:"Huge (Slower-Paced)"}]
      scope.starting_length_options = [{value:0,name:"5 (Tricky with AI)"},{value:1,name:"10 (Easier)"},{value:2,name:"20 (Recommended)"},{value:3,name:"50 (Messy)"}]
      scope.expand_length_options = [{value:0,name:"5 (Very Easy)"},{value:1,name:"5 (Easy)"},{value:2,name:"10 (Recommended)"},{value:3,name:"50 (Huge Mess)"}]      
      scope.candy_buffer_options = [{value:0,name:"No Buffer (Spawn Anywhere)"},{value:1,name:"1 (Spawn 1+ Blocks from Walls)"},{value:2,name:"2 (Spawn 2+ Blocks from Walls)"},{value:3,name:"3 (Spawn 3+ Blocks from Walls)"}]

      /* Player and AI Starting Locations (same for every map) */
      var starting_locations = [[0.2,0.13],[0.8,0.87],[0.2,0.87],[0.8,0.13],[0.9,0.6],[0.9,0.4],[0.1,0.6],[0.1,0.4]];  

      /* Point Calculation Items */
      var length_bonus = [1,1,2,4];
      var speed_bonus = [0,20,50,100];
      var speed_snake_bonus = [5,10,20,100];
      var dimension_bonus = [7,3,2];
      var speed_dimension_bonus = [1,2,5,10];
      var candy_dimension_bonus = [30,20,10];
      ...
      function Candy(collidable_hash,candy_buffer) {
        ...
        this.reset_candy = function() {
          if ( !first) collidable_hash.remove(this);
          this.findSpawn();
        }
      }
      ...
      function CollidableHash(ctx,block_size,w,h) {
        ...
        /* Remove a Collidable from the hash */
      ...
      function CircularCollidablePoolQueue(initial_size,collidable_hash,collidable_constructor) {
        this.buffer = _.range(initial_size).map(function(){return null});
        ...
        this.expand = function(expand_amount) {
          ...
          Array.prototype.splice.apply(this.buffer, [this.head,0].concat(_.range(expand_amount).map(function(){return null})));  
      ...
      function AISnake(initial_size,expand_size,collidable_hash) {
        ...
        this.compute_action = function(player_x,player_y,candy_x,candy_y) {
          ...
          if ( this.snake_type === AI_SNAKE_ASSASSIN_TYPE ) {
            goal_x = player_x;
            goal_y = player_y;
          // Smart (Purple) Snakes go for either the player or the food, whichever is closest
          } else if ( this.snake_type === AI_SNAKE_CLOSEST_TYPE ) {
      ...
      var init = function() {
          ...
          ai_snake = _.range(scope.ai_snakes).map(function(){ return new AISnake(start_length,expand_length,playing_field) });
